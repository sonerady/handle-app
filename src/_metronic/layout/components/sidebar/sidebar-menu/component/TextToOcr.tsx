...
const TextToImage: FC = () => {
  const {createOcr} = useAuthService()
  const {
    imageData,
    setImageData,
    setLoading,
  ...
  const handleCreatePost = async (postData: any) => {
    if (postData) {
      try {
        ...
        const data = await createOcr(formData)
        if (data) {
        ...
        const uploadedImageData = data?.image?.[0]
        if (uploadedImageData) {
          setImagePreview(`data:image/jpeg;base64,${uploadedImageData}`)
        }
