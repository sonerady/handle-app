...
import CodeLogo from '../../../_metronic/assets/CodeLogo.svg'
import {ReactMarkdown} from 'react-markdown/lib/react-markdown'

const TextToCode: FC = () => {
  const {createCode, createCodeChatGoogle} = useAuthService()
  const {setLoading, setCode, code, balance, setShowGlobalAlert, setVersion, version} = useGlobal()
  ...
  const handleCreateCode = async (data: any): Promise<{content: string}> => {
    if (data) {
      try {
        const datas = version === 'v2' ? await createCodeChatGoogle(data) : await createCode(data)
        setCode(datas)
        setLoading(false)
        return datas
      } catch (error) {
    ...
    >
      ...
      {!messages.length && (
        <div className='row d-flex justify-content-center align-items-center h-100'>
          <img style={{opacity: '0.1', width: '280px'}} src={CodeLogo} alt='Hyper Logo' />
        </div>
      )}
